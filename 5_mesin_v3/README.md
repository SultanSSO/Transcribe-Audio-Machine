# 🎙️ Mesin Notulen v3 — Mesin Transkripsi & Notulensi Otomatis
...
**Mesin Notulen v3** adalah mesin otomatis untuk:

1. **Transkripsi audio → teks** (speech-to-text)
2. **Output utama saat ini: TXT + JSON**
3. **Konfigurasi lama notulen Markdown (MD) tetap tersimpan** dan bisa dipakai ulang dengan opsi `--with-md`
4. **Konversi notulen MD → dokumen DOCX** profesional bila mode legacy MD digunakan

Digunakan oleh tim BTD UGM untuk membuat notulensi rapat, FGD, sidang, podcast, dan kegiatan lainnya secara efisien.

> **Catatan versi:** v3 dibangun ulang dengan mengadopsi setup bersih dari v2 (`4_notulen_enggine_v2`). Setup minimalis: hanya 3 dependensi.
...
│   └── XX_nama/                  # Auto-created per audio
├── zip_archive/                  # Arsip file zip
├── transcribe.py                 # 🎤 Mesin transkripsi (audio → TXT + JSON; MD opsional dengan --with-md)
...
| `Notulen_*.md` | Opsional/legacy, hanya dibuat jika menjalankan `transcribe.py --with-md` |
| `Notulen_*.docx` | Opsional/legacy, dibuat dari MD dengan `generate_docx.py` |
...
| **Audio Decode** | ffmpeg (system) + NumPy | Gyan / numpy |
...
```powershell
cd "d:\SULTAN NAUFAL\KULIAH\MAGANG\MAGANG BTD\proyek\workspace\5_mesin_v3"
```
...
1. Membuat folder baru `transcribe_hasil/01_rapat_bulanan/` (auto-number)
2. Download model `small` (sekali saja, ~970 MB)
3. Transkripsi audio → teks
4. Menghasilkan 2 file utama: `transkrip.txt`, `transkrip.json`

> Konfigurasi lama pembuatan notulen MD tidak dihapus. Gunakan `--with-md` jika nanti perlu membuat `Notulen_*.md` lagi tanpa install ulang dependensi.
...
# Buat notulen MD legacy jika diperlukan
python transcribe.py audio.mp3 --with-md
...
options:
  ...
  --with-md          Buat Notulen_*.md legacy selain TXT + JSON
```
...
python generate_docx.py --folder 01_rapat_bulanan

# Konversi file MD spesifik
python generate_docx.py --md "transcribe_hasil/01_rapat_bulanan/Notulen_rapat_bulanan.md"
...
flowchart LR
    ...
    C --> D[transkrip.txt]
    C --> E[transkrip.json]
    C -->|opsional: --with-md| F[Notulen_*.md]
...
# 3. Jika perlu notulen MD legacy, jalankan ulang dengan --with-md
python transcribe.py "audio_archive/rapat_koordinasi.mp3" --model small --with-md

# 4. Review & edit Notulen_rapat_koordinasi.md
#    (buka di VS Code, perbaiki kesalahan transkrip, lengkapi info)

# 5. Konversi ke DOCX
python generate_docx.py --folder 01_rapat_koordinasi

# 6. Final check DOCX di Word, lalu distribusikan
```
...
### Q: Apakah v3 bisa membaca format audio yang sama dengan v2?
